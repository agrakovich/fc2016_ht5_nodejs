...
const AccessTokenModel    = require('./models/authentification').AccessTokenModel;
const RefreshTokenModel   = require('./models/authentification').RefreshTokenModel;
...
server.exchange(oauth2orize.exchange.password(function(client, username, password, scope, done) {
    UserModel.findOne({ username: username }, function(err, user) {
        ...
        const tokenValue = bcrypt.randomBytes(32).toString('base64');
        const refreshTokenValue = bcrypt.randomBytes(32).toString('base64');
...
// token endpoint
exports.token = [
    passport.authenticate(['basic', 'oauth2-src-password'], { session: false }),
    server.token(),
    server.errorHandler()
]